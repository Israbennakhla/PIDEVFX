...
public class ModifierReclamation implements Initializable {
    ...
    private boolean validerChamps() {
        ...
        if (champSujet.getText().trim().isEmpty()) {
            errSujet.setText("⚠️ Le sujet est obligatoire.");
            ok = false;
        } else if (champSujet.getText().trim().length() < 5) {
            errSujet.setText("⚠️ Minimum 5 caractères.");
            ok = false;
        }

        // Description
        if (champDescription.getText().trim().isEmpty()) {
            errDescription.setText("⚠️ La description est obligatoire.");
            ok = false;
        } else if (champDescription.getText().trim().length() < 10) {
            errDescription.setText("⚠️ Minimum 10 caractères.");
            ok = false;
        }

        // Priorité
        if (champPriorite.getValue() == null) {
            errPriorite.setText("⚠️ Choisissez une priorité.");
            ok = false;
        }

        // Nom
        if (champNom.getText().trim().isEmpty()) {
            errNom.setText("⚠️ Le nom est obligatoire.");
            ok = false;
        }

        // Email
        if (champEmail.getText().trim().isEmpty()) {
            errEmail.setText("⚠️ L'email est obligatoire.");
            ok = false;
        } else if (!champEmail.getText().contains("@")) {
            errEmail.setText("⚠️ Email invalide (manque @).");
            ok = false;
        } else if (!champEmail.getText().contains(".")) {
            errEmail.setText("⚠️ Email invalide (manque domaine).");
            ok = false;
        }
