...
public class CreerAnnonceController {
    ...
    private void chargerAnimaux() {
        ...
        if (uid <= 0) return;
        petsList.addAll(new ServicePet().getByOwnerId(uid));
        for (Pet p : petsList) {
    ...
    private String buildMapHtml() {
        return "<!DOCTYPE html><html><head>" +
                ...
                "  maxZoom: 19" +
                "}).addTo(map);" +
                // Leaflet a parfois besoin d'invalider sa taille après le rendu WebView.
                "setTimeout(function(){ try { if (map && map.invalidateSize) map.invalidateSize(); } catch(e) {} }, 200);" +
                ...
                // Fonction reset appelable depuis Java (exposée sur window)
    ...
    private void handleResetAdresse() {
        ...
        if (webEngine != null) {
            webEngine.executeScript("if (window.resetMarker) window.resetMarker();");
        }
    }
    ...
    private void handleCreer() {
        ...
        Announcement annonce = new Announcement(
                ...
                fieldService.getText().trim(), petId, currentUserId());
    ...
    private void fermerFenetre() {
        Platform.runLater(() -> {
            Stage s = (Stage) fieldService.getScene().getWindow();
            s.close();
        });
    }
