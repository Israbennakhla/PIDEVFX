...
public class MesAnnoncesController {
    ...
    public void initialize() {
        ...
        notifBadge.setChatInfo(0, "Moi", "Gardien / propriétaire");
    ...
    private void onNotifCliquee(Notification n) {
        ...
        if ("NOUVELLE_POSTULATION".equals(n.getType())) {
            ...
            String nomGardien = serviceUser.getDisplayNameById(n.getExpediteurId());
            dialog.setHeaderText(nomGardien + " veut garder " + petName);
    ...
    private Announcement trouverAnnonce(int id) {
        for (Announcement a : allAnnonces) {
            if (a.getId() == id) return a;
        }
        return serviceAnn.getById(id);
    }
    ...
    private void chargerPets() {
        ...
        int uid = currentUserId();
        if (uid <= 0) return;
        for (Pet p : new ServicePet().getByOwnerId(uid)) {
    ...
    private HBox buildAdresseRow(String adresse) {
        String adr = (adresse == null || adresse.trim().isEmpty()) ? "—" : adresse;

        Label lbl = new Label("Adresse :");
        lbl.setStyle("-fx-font-size:13px;-fx-font-weight:bold;-fx-text-fill:#1a1a2e;-fx-min-width:160px;");
        Label val = new Label(adr); val.setStyle("-fx-font-size:13px;-fx-text-fill:#5a5a8a;");
        val.setWrapText(true); val.setMaxWidth(340);
        HBox row = new HBox(6, lbl, val); row.setAlignment(Pos.CENTER_LEFT);
        if (!"—".equals(adr) && !adr.isEmpty()) {
            ...
            btnMap.setOnAction(e -> ouvrirCarte(adr));
    ...
    private void loadData() {
        int uid = currentUserId();
        allAnnonces.setAll(uid > 0 ? serviceAnn.getByUserId(uid) : List.of());
        currentPage = 1;
        applyFilters();
    }
    ...
    private void handleNouvelle() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/sitmypet/fxml/CreerAnnonce.fxml"));
            Parent root = loader.load();
            Stage dlg = new Stage();
            dlg.initOwner(listAnnonces.getScene().getWindow());
            dlg.initModality(Modality.WINDOW_MODAL);
            dlg.setTitle("Créer une annonce");
            dlg.setScene(new Scene(root, 920, 820));
            dlg.setOnHidden(e -> {
                chargerPets();
                loadData();
            });
    ...
    private void handleModifier(Announcement a) {
        try {
            ...
            dlg.initOwner(listAnnonces.getScene().getWindow());
            dlg.initModality(Modality.WINDOW_MODAL);
            dlg.setTitle("Modifier une annonce");
            dlg.setScene(new Scene(root, 640, 720));
    ...
    private void stop() {
        if (notifBadge != null) notifBadge.stopPolling();
    }
}